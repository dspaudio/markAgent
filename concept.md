...
AI 에이전트(Claude Code, Gemini CLI 등)와 CLI 환경에서 매끄럽게 연동되는 초경량 마크다운 에디터/뷰어 기획안입니다.
...
* **가격 전략:** $0.99 (충동 구매를 유도하는 압도적 가성비 및 초기 사용자 확보)
...
* **Dual Mode:** 에이전트의 출력을 보는 **Preview 모드**와 직접 수정하는 **Edit 모드**의 조화.

### 3. AI 컨텍스트 레이어 (Context Manager)
* **Prompt Snippets:** AI가 이해하기 좋은 구조(Task, Context 등)의 마크다운 템플릿 지원.
...
- [ ] AI 프롬프트 최적화 스니펫 및 템플릿 엔진 추가
- [ ] 인라인 Diff 하이라이트 표시

### **Phase 3: Polishing & Launch (상용화)**
- [ ] Mermaid(다이어그램) 및 KaTeX(수식) 렌더링 지원
- [ ] 개발자 친화적 테마(Dracula, Nord 등) 및 커스텀 CSS
- [ ] LLM 입력 비용 예측을 위한 토큰 카운터 기능
- [ ] Mac App Store 등록 ($0.99) 및 CLI 바이너리 배포

---

## 🚀 마케팅 타겟 (Target Audience)
1. **Claude Code / Gemini CLI** 헤비 유저
