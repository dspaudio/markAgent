...
UI, 렌더링, 파일 미리보기, 에디터, 파일 감시, 비동기 Task, 이미지, 터미널, 캐시 등 메모리 민감 영역을 수정한 경우 **메모리 누수 점검을 테스트의 필수 항목**으로 포함한다.

- 가능한 경우 집중 자동화 테스트로 누수 재발 가능성을 먼저 잠근다.
- 실제 macOS 앱 표면에서만 확인 가능한 흐름은 QA 앱 실행 후 `leaks <pid>` 같은 프로세스 레벨 점검을 수행한다.
- 누수 점검을 실행하지 못한 경우, 이유와 남은 위험을 결과 보고에 명확히 남긴다.

### libghostty-spm 의존성 확인 (필수)

터미널 뷰, 선택 영역 복사, 스니펫 저장, 키 바인딩, pasteboard, 또는 `Vendor/libghostty-spm` 관련 작업을 할 때는 `libghostty-spm` 원 라이브러리에 올린 선택 영역 복사 누수 수정 PR이 반영되었는지 확인한다.

- `Lakr233/libghostty-spm`의 기본 브랜치 또는 사용하는 릴리스 버전에 해당 PR의 수정이 포함되었는지 확인한다.
- PR이 아직 반영되지 않았거나 사용하는 버전에 포함되지 않았다면, MarkAgent의 로컬 vendor 패치 또는 동등한 워크어라운드를 제거하지 않는다.
- PR 반영을 확인한 뒤 원격 의존성으로 되돌리거나 vendor 패치를 제거하는 경우, 실제 터미널 선택 영역 복사 흐름에서 스니펫 저장과 `leaks <pid>` 검증을 다시 수행한다.
- 확인 결과와 기준 버전 또는 커밋을 작업 결과에 명확히 기록한다.
