...
- **Ghostty terminal tabs:** Embedded terminal tabs powered by `libghostty-spm` for handling multiple work sessions in one window. Terminal, Markdown, and Git Diff tabs created from the same work session stay grouped together, and Cmd+1 through Cmd+9 switch or cycle through those groups. Inactive terminal tabs are marked as hidden to reduce unnecessary Ghostty Metal redraw work and lower energy usage when multiple sessions are open.
...
- **Ghostty 터미널 탭:** `libghostty-spm`을 사용한 내장 터미널 탭으로 여러 작업 세션을 한 창에서 다룹니다. 같은 작업 세션에서 열린 터미널, Markdown, Git Diff 탭은 하나의 그룹으로 묶이고, Cmd+1부터 Cmd+9까지의 단축키로 그룹 전환 또는 그룹 내부 순환을 할 수 있습니다. 비활성 터미널 탭은 숨김 surface로 표시해 여러 세션을 열어 둔 상태에서 불필요한 Ghostty Metal 재렌더링과 에너지 사용을 줄입니다.
