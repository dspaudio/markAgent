...
- **Change tracking:** Shows changed files from the Git repository in the sidebar with per-file `+/-` line counts, supports branch checkout and Git init from the titlebar, keeps the full current branch name visible in the titlebar, and opens a GitHub-style Files changed tab that displays all changed file diffs in one scrollable view. Selecting a file in the right sidebar jumps the diff tab to that file's section.
