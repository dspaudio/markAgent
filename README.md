...
- **AI subscription and system status:** Registers Claude and Codex usage in Settings, shows current usage windows and reset times in the bottom status bar, and refreshes active providers on a bounded 15-minute cadence with staleness checks, retry backoff, duplicate-request suppression, and lifecycle-safe wakeups. The same status bar includes a MarkAgent-owned Caffeinate toggle and live process resident-memory usage.
- **Project workspaces:** Keeps a persistent project list in a full-height left sidebar and gives every project its own visible tab strip, active terminal/content, and right-utility state. Switching projects preserves inactive terminal sessions while removing their tabs, focus, hit testing, and accessibility nodes from the active workspace. The responsive three-column shell keeps the project list, central work surface, and right utility aligned, restores saved window sizes safely, and collapses side regions before squeezing the terminal below its supported minimum.
- **Ghostty terminal tabs:** Embedded terminal tabs powered by `libghostty-spm` for handling multiple work sessions in one window. Terminal, Markdown, and Git Diff tabs created from the same work session stay grouped together, and Markdown files opened from an active terminal group are inserted next to that group instead of being appended after unrelated tabs. Child tabs tuck behind their group parent when inactive, expand with an animation when the parent is active, and Cmd+1 through Cmd+9 switch or cycle through those groups. Inactive terminal tabs are marked as hidden to reduce unnecessary Ghostty Metal redraw work and lower energy usage when multiple sessions are open.
...
- **AI 구독 및 시스템 상태:** Settings에서 Claude와 Codex 사용량을 등록하고 하단 상태 바에서 현재 사용량 구간과 reset 시간을 확인합니다. 활성 provider는 15분 주기로 갱신하되 staleness 확인, 실패 backoff, 중복 요청 차단, 앱 수명주기에 안전한 wake 처리를 적용합니다. 같은 상태 바에서 MarkAgent가 소유한 Caffeinate를 켜고 끄며 현재 프로세스 resident memory도 확인할 수 있습니다.
- **프로젝트 워크스페이스:** 전체 높이의 왼쪽 사이드바에 프로젝트 목록을 유지하고 프로젝트마다 보이는 탭 strip, 활성 터미널/콘텐츠, 오른쪽 유틸리티 상태를 독립적으로 보존합니다. 프로젝트를 전환해도 비활성 터미널 세션은 유지되지만 해당 탭과 focus, hit testing, 접근성 node는 현재 workspace에서 제외됩니다. 반응형 3열 shell은 프로젝트 목록, 중앙 작업 영역, 오른쪽 유틸리티를 정렬하고 저장된 창 크기를 안전하게 복원하며, 터미널이 지원 최소 폭보다 좁아지기 전에 양쪽 영역을 자동으로 접습니다.
- **Ghostty 터미널 탭:** `libghostty-spm`을 사용한 내장 터미널 탭으로 여러 작업 세션을 한 창에서 다룹니다. 같은 작업 세션에서 열린 터미널, Markdown, Git Diff 탭은 하나의 그룹으로 묶이고, 활성 터미널 그룹에서 연 Markdown 파일은 무관한 탭 뒤가 아니라 해당 그룹 옆에 삽입됩니다. 자식 탭은 비활성 상태에서 부모 탭 뒤로 접혀 보이고, 부모 탭이 활성화되면 애니메이션으로 펼쳐지며, Cmd+1부터 Cmd+9까지의 단축키로 그룹 전환 또는 그룹 내부 순환을 할 수 있습니다. 비활성 터미널 탭은 숨김 surface로 표시해 여러 세션을 열어 둔 상태에서 불필요한 Ghostty Metal 재렌더링과 에너지 사용을 줄입니다.
